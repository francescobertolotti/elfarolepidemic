...
class conclusions:
    ...
    def Chart(self, par, gv, max_line: bool = True, threshold_line: bool = True, cont_threshold_line: bool = True, contNotPres_threshold_line: bool = True):
        ...
        plt.plot(arr_x, gv.contagious_history, label="Contagious people (%d weeks)" % par.infection_duration)
        ...
        if cont_threshold_line:
            line_threshold = plt.Line2D((0, arr_x[len(arr_x) - 1]), (par.infection_threshold * par.n_persons, par.infection_threshold * par.n_persons), color="blue", label=f"Contagious treshold: {par.infection_threshold * par.n_persons}")
            plt.gca().add_line(line_threshold)
        if contNotPres_threshold_line:
            line_threshold = plt.Line2D((0, arr_x[len(arr_x) - 1]), (par.infection_thresholdNotPresent * par.n_persons, par.infection_thresholdNotPresent * par.n_persons), color="lightblue", label=f"Contagious treshold (not present): {par.infection_thresholdNotPresent * par.n_persons}")
            plt.gca().add_line(line_threshold)
